...
class UpdateManagerChecker():
    ...
    def get_models() -> list[UpdateManager]:
        return [StaticFileUpdater, GithubUpdater, GitlabUpdater, CodebergUpdater, Forgejo]
...
class ForgejoUpdater(UpdateManager):
    ...
    def get_url_data(url: str):
        # Example: https://git.citron-emu.org/Citron/Emulator/releases/download/0.11.0/citron_stable=0.110.0_v3_linux.AppImage
        paths = []
        if url.startswith('https://'):
